...
## 2026-07-23 - Best-of-Three Arena Rounds

### The Change
- Added 100-HP round state, a visible 180-second clock, round-win scores, and a maximum of three rounds to the Flutter PvP arena.
- Added a winner announcement and three-count transition between rounds, with both fighters reset to 100 HP before the next round.
- Changed combo damage to exactly 5/10/15 for `x1`/`x2`/`x3`, retained the 100-HP heal cap, and replaced the `PROJ 1` label with the `x1 → x2 → x3` progression.
- Updated the result card to show round wins and synchronized online round/combo fields from server events.
- Added unit and widget coverage for exact combo damage, timeout resolution, two-win match completion, three-round draws, countdown transitions, and the updated arena HUD.

### The Reasoning
- Round ownership lives in the controller so combat input, bot actions, pause behavior, timers, and result navigation all observe one lifecycle.
- The opening and between-round countdowns reuse the arena overlay, while online matches defer authoritative resolution to the game server.
- HP determines timeout winners directly; equal HP produces a tied round so damage-point history cannot override the visible combat result.

### The Tech Debt
- The round countdown is a fixed three-count animation rather than a server timestamp animation; online state remains authoritative if network delivery arrives slightly before or after the visual countdown.

## 2026-07-23 - Arena UI Alignment Refinements

### The Change
- Simplified the combo badge to show only its current multiplier (`x1`, `x2`, or `x3`) beneath the combo label.
- Changed the player-match card to use the same full-size portrait frame as the bot-match card.
- Moved the tower loadout label above its preview to match the character preview layout.

### The Reasoning
- The current combo state is faster to read without displaying future levels, and matching portrait and loadout layouts keeps the arena setup screens visually consistent.

### The Tech Debt
- The mode cards currently use one representative portrait each; a future two-player composition should preserve the same square frame instead of splitting it into narrow columns.
