...
## 2026-07-23 - Server-Authoritative Best-of-Three Matches

### The Change
- Extended the shared battle contract with combo levels, projectile level, current round, remaining round time, round-win scores, round-break phase, and player-relative last-round outcome.
- Changed online damage to `x1 = 5`, `x2 = 10`, and `x3 = 15`, with a seven-second server-side combo expiry and 100-HP clamping.
- Added 180-second server round timers, three-second round breaks, HP/card/combo reset at the next round, first-to-two victory, and a hard three-round limit.
- Added `round_timeout` result persistence support to bootstrap SQL and a forward Supabase migration.
- Updated engine and service regression tests for combo scaling, HP-zero round transitions, timeout winners, two-win completion, and three tied rounds.

### The Reasoning
- The server remains the only authority for online damage and match completion; the mobile timer is a display mirror and cannot award an online round.
- HP-zero and timeout resolution share the same round finalizer so scores, transitions, cleanup, and final match persistence cannot diverge.
- Per-room round and break timers are cleared and replaced at every transition to prevent an expired timer from affecting a later round.

### The Tech Debt
- Apply `20260723000000_add_round_timeout_match_reason.sql` to deployed Supabase environments before persisting timeout-ended matches.
- Round timers are process-local like the existing card and bot timers; horizontal scaling will require a shared scheduler or durable room clock recovery.
