...
-- Daily mission and streak reminder notifications
...
create unique index if not exists push_installations_fcm_token_unique on public.push_installations(fcm_token);
create index if not exists push_installations_active_idx on public.push_installations(active, authorized, last_seen_at desc);

create table if not exists public.notification_deliveries (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles(id) on delete cascade,
  installation_id uuid not null,
  kind text not null check (kind in ('morning', 'rescue')),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint notification_delivery_dedupe_unique unique (user_id, installation_id, kind, local_date)
);
...
revoke all on public.notification_preferences, public.push_installations, public.notification_deliveries from public, anon, authenticated;
grant all on public.notification_preferences, public.push_installations, public.notification_deliveries to service_role;
...
begin
  ...
  insert into public.notification_deliveries (user_id, installation_id, kind, local_date, business_date, expires_at)
  select i.user_id, i.installation_id, 'morning', local_clock.local_date, v_business_date, v_expires_at
  from public.push_installations i
  join public.notification_preferences preferences on preferences.user_id = i.user_id
  cross join lateral (
    select timezone(i.time_zone, p_now) as local_now, timezone(i.time_zone, p_now)::date as local_date
  ) local_clock
  where preferences.enabled and preferences.morning_enabled and i.active and i.authorized
    ...
    and (
      select count(*) from public.daily_mission_progress mission
      where mission.user_id = i.user_id and mission.business_date = v_business_date
    ) < 2
  on conflict on constraint notification_delivery_dedupe_unique do nothing;

  insert into public.notification_deliveries (user_id, installation_id, kind, local_date, business_date, expires_at)
  select i.user_id, i.installation_id, 'rescue', local_clock.local_date, v_business_date, v_expires_at
  from public.push_installations i
  join public.notification_preferences preferences on preferences.user_id = i.user_id
  ...
  where preferences.enabled and preferences.rescue_enabled and i.active and i.authorized
    and i.last_seen_at >= p_now - interval '30 days'
    and profile.current_streak > 0 and profile.last_streak_date = v_business_date - 1
    and not exists (
      select 1 from public.daily_learning_activity activity
      where activity.user_id = i.user_id and activity.business_date = v_business_date
    )
  ...
  where delivery.status in ('pending', 'processing') and delivery.expires_at <= p_now;
  ...
  with claimed as (
    ...
    where ((delivery.status = 'pending' and delivery.next_attempt_at <= p_now)
      ...
      and delivery.attempt_count < 3 and delivery.expires_at > p_now
      and case delivery.kind
        when 'morning' then (
          select count(*) from public.daily_mission_progress mission
          where mission.user_id = delivery.user_id and mission.business_date = delivery.business_date
        ) < 2
        when 'rescue' then exists (
          select 1 from public.profiles profile
          where profile.id = delivery.user_id and profile.current_streak > 0
            and profile.last_streak_date = delivery.business_date - 1
            and not exists (
              select 1 from public.daily_learning_activity activity
              where activity.user_id = delivery.user_id and activity.business_date = delivery.business_date
            )
        )
        else false
      end
    ...
    set status = 'processing', attempt_count = delivery.attempt_count + 1,
        lease_until = p_now + interval '5 minutes', updated_at = p_now
    from claimed where delivery.id = claimed.id returning delivery.*
  )
  select leased.id, leased.user_id, leased.installation_id,
    installation.fcm_token, installation.platform, installation.time_zone,
    leased.kind, leased.local_date, leased.business_date, profile.current_streak,
    array(
      select expected.mission_key
      from unnest(array['daily_practice', 'daily_pvp']) as expected(mission_key)
      where not exists (
        select 1 from public.daily_mission_progress progress
    ...
    leased.expires_at, leased.attempt_count
  from leased
  join public.push_installations installation
    on installation.user_id = leased.user_id and installation.installation_id = leased.installation_id
  join public.profiles profile on profile.id = leased.user_id;
end;
$$;

revoke all on function public.claim_due_notification_deliveries(timestamptz, integer) from public, anon, authenticated;
